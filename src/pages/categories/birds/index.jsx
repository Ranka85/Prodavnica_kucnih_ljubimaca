...
export const BirdsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAdsByPetType('Birds');
  ...
  const birdsToDisplay = data ? data.slice((currentPage - 1) * 8, currentPage * 8) : [];
  ...
  return (
    <div className="container">
      ...
      {data && (currentPage - 1) * 8 + birdsToDisplay.length < data.length && ( 
          <button
            onClick={() => setCurrentPage(currentPage + 1)}
            className="button-prev-next"
          >
