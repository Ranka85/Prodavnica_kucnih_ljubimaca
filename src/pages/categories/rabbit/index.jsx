...
export const RabbitPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getAdsByPetType('Rabbit');
  ...
  const rabbitsToDisplay = data ? data.slice((currentPage - 1) * 8, currentPage * 8) : [];
  ...
  return (
    <div className="container">
      ...
      {data && (currentPage - 1) * 8 + rabbitsToDisplay.length < data.length && ( 
          <button
            onClick={() => setCurrentPage(currentPage + 1)}
            className="button-prev-next"
          >
